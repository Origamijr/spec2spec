...
def train(generator,
    ...
    def train_discriminator(fake, fake_source, real):
        ...
        loss_D_fake = criterion_GAN(pred_fake, torch.zeros(pred_fake.shape).to(device))
        # Real
        real_AB = torch.cat((fake_source, real), 1)
        pred_real = discriminator(real_AB)
        loss_D_real = criterion_GAN(pred_real, torch.ones(pred_real.shape).to(device))
    ...
    def train_generator(fake, fake_source, real):
        ...
        fake_AB = torch.cat((fake_source, fake), 1)
        pred_fake = discriminator(fake_AB.detach())
        loss_G_GAN = criterion_GAN(pred_fake, torch.ones(pred_fake.shape).to(device))
        # Reconstruction
